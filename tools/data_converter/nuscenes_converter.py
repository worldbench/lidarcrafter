...
from lidargen.dataset.map_utils.nuscmap_extractor import NuscMapExtractor
...
def _fill_trainval_infos(nusc,
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        info = {
            'lidar_path': lidar_path.split("nuscenes", 1)[1][1:],
            'token': sample['token'],
            'sweeps': [],
            'cams': dict(),
        ...
        # prev token lidar info
        prev_info = {}
        if sample['prev'] != '':
            prev_info['valid'] = True
            prev_info['token'] = sample['prev']
            next_sample = nusc.get('sample', sample['prev'])
            next_lidar_token = nusc.get('sample_data', next_sample['data']['LIDAR_TOP'])
            next_sample_info = obtain_sensor2top(nusc, next_lidar_token['token'], l2e_t,
                                        l2e_r_mat, e2g_t, e2g_r_mat, 'lidar')
            prev_info.update(**next_sample_info)

        else:
            prev_info['valid'] = False
        info['prev_info'] = prev_info
        ...
        if not test:
            ...
            gt_agent_fut_states = ['Unkonwn'] * num_box
            for i, anno in enumerate(annotations):
                ...
                if fut_traj_local.shape[0] > 0:
                    ...
                    distance = np.linalg.norm(
                        fut_traj_local[-1] - fut_traj_local[0])
                    # if the distance is too small, we ignore this agent
                    if fut_traj_local[-1][0] >= 2:
                        gt_agent_fut_states[i] = 'Turn Right'  # Turn Right
                    elif fut_traj_local[-1][0] <= -2:
                        gt_agent_fut_states[i] = 'Turn Left' # Turn Left
                    elif distance < 0.5:
                        gt_agent_fut_states[i] = 'Stop'  # Stop
                    else:
                        gt_agent_fut_states[i] = 'Go Straight'  # Go Straight
            ...
            ego_fut_trajs = np.dot(rot_mat, ego_fut_trajs.T).T
            distance = np.linalg.norm(
                ego_fut_trajs[-1] - ego_fut_trajs[0])
            # drive command according to final fut step offset
            if ego_fut_trajs[-1][0] >= 2:
                command = np.array([1, 0, 0])  # Turn Right
                gt_ego_fut_state = 'Turn Right'
            elif ego_fut_trajs[-1][0] <= -2:
                command = np.array([0, 1, 0])  # Turn Left
                gt_ego_fut_state = 'Turn Left'
            elif distance < 0.5:
                gt_ego_fut_state = 'Stop'  # Stop
            else:
                command = np.array([0, 0, 1])  # Go Straight
                gt_ego_fut_state = 'Go Straight'
            # get offset
            ...
            info['gt_agent_fut_states'] = gt_agent_fut_states
            ...
            info['gt_ego_fut_state'] = gt_ego_fut_state
