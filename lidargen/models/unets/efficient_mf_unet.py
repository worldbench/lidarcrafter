...
class SelfAttentionBlock(nn.Module):
    def __init__(
        ...
        self.attn = Attention(in_channels, num_heads=num_heads, qkv_bias=True, qk_norm=True, norm_layer=RMSNorm)
...
class MFEfficientUNet(nn.Module):
    """
    ...
    def __init__(
        ...
        # timestep embedding
        self.start_time_embedding = nn.Sequential(
        ...
        self.end_time_embedding = nn.Sequential(
            ops.SinusoidalPositionalEmbedding(base_channels),
            nn.Linear(base_channels, temb_channels),
            nn.SiLU(),
            nn.Linear(temb_channels, temb_channels),
    ...
    def forward(self, 
                ...
                end_timesteps: torch.Tensor,
                condition=None
                ) -> torch.Tensor:
        h = images

        # timestep embedding
        if len(start_timesteps.shape) == 0:
            start_timesteps = start_timesteps[None].repeat_interleave(h.shape[0], dim=0)
        if len(end_timesteps.shape) == 0:
            end_timesteps = end_timesteps[None].repeat_interleave(h.shape[0], dim=0)
        start_temb = self.start_time_embedding(start_timesteps.to(h))
        end_temb = self.end_time_embedding(end_timesteps.to(h))
        temb = start_temb + end_temb
