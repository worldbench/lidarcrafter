...
import math
from .graph import GraphTripleConvNet, _init_weights
from .ldm_diffusion_util import (
    checkpoint,
    conv_nd,
    linear,
    avg_pool_nd,
...
from .attention import SpatialTransformer, SpatialTransformer1D
    ...
    def forward(self, x, emb, context=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer) or isinstance(layer, SpatialTransformer1D):
                x = layer(x, context)
            else:
                x = layer(x)
        return x
...
class UNet1DModel(nn.Module):
    ...
    def __init__(
        ...
        context_dim = crossattn_dim if conditioning_key in ['crossattn', 'hybrid'] else None
            ...
            for _ in range(num_res_blocks):
                ...
                if ds in attention_resolutions:
                    ...
                    layers.append(
                        ...
                        ) if not use_spatial_transformer else SpatialTransformer1D(
    ...
    def forward(self, box_t, cond_dict):
        hs = []
        timesteps = cond_dict['time_condition']
        layout_outputs = cond_dict['other_condition']

        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        context = layout_outputs['c_b']
        latent_box_rel = self.box_messsage_passing(layout_outputs['uc_b'], layout_outputs['preds'], box_t, t_emb=emb, enable_t_emb=self.enable_t_emb)
        ...
        for module in self.output_blocks:
            ...
            h = module(h, emb, context)
        return self.out(h).squeeze(-1)  # Remove the time dimension