...
class Upsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        self.dims = dims
        if use_conv:
            self.conv = conv_nd_range(dims, self.channels, self.out_channels, 3, padding=1, ring=True)
...
class Downsample(nn.Module):
    ...
    def __init__(self, channels, use_conv, dims=2, out_channels=None):
        ...
        if use_conv:
            self.op = conv_nd_range(
                dims, self.channels, self.out_channels, 3, stride=stride, padding=1, ring=True
            )
        else:
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.in_layers = nn.Sequential(
            ...
            SiLU(),
            conv_nd_range(dims, channels, self.out_channels, 3, padding=1, ring=True),
        )

        self.updown = up or down

        if up:
            self.op = ops.Resample(up=2, ring=True)
        elif down:
            self.op = ops.Resample(down=2, ring=True)
        else:
            self.op = nn.Identity()
        ...
        self.out_layers = nn.Sequential(
            ...
            zero_module(
                conv_nd_range(dims, self.out_channels, self.out_channels, 3, padding=1, ring=True)
            ),
        )
        ...
        if self.out_channels == channels:
            ...
            self.skip_connection = conv_nd_range(
                dims, channels, self.out_channels, 3, padding=1, ring=True
            )
        else:
...
class LayoutUnetV1(nn.Module):
    ...
    def __init__(
        ...
        self.time_embed = nn.Sequential(
            ops.SinusoidalPositionalEmbedding(model_channels),
        ...
        self.input_blocks = nn.ModuleList(
            [TimestepEmbedSequential(conv_nd_range(dims, in_channels, ch, 3, padding=1, ring=True))]
        )
        ...
        self.out = nn.Sequential(
            normalization(ch),
            SiLU(),
            zero_module(conv_nd_range(dims, input_ch, out_channels, 3, padding=1, ring=True)),
        )
    ...
    def forward(self, x, cond_dict):
        hs, extra_outputs = [], []
        timesteps = cond_dict['time_condition']
        layout_outputs = cond_dict['other_condition']
        ...
        h = x.type(self.dtype)
        if 'concat_cond' in layout_outputs.keys():
            h = torch.cat([h, layout_outputs['concat_cond']], dim=1)
