...
sys.path.append('/home/e/e1493786/AlanLiang/Projects/AlanLiang/LidarGen4D')
...
def train(args):
    ...
    if cfg.resume is not None:
        flow, model, lidar_utils, global_step, optimizer_dict, lr_scheduler_dict = inference.load_model_flow_training(cfg)

    else:
        flow, model, lidar_utils = inference.load_model_flow_training(cfg)
    ...
    flow, optimizer, dataloader, lr_scheduler = accelerator.prepare(
        flow, optimizer, dataloader, lr_scheduler
    )
    ...
    while global_step < cfg.training.num_steps:
        ...
        for batch in dataloader:
            ...
            if accelerator.is_main_process:
                flow_ema.update()
                log["ema/decay"] = flow_ema.get_current_decay()

                if global_step == 1:
                    log_images(x_0, "image", global_step)

                # if global_step % cfg.training.steps_save_image == 0:
                #     flow_ema.ema_model.eval()
                #     sample = flow_ema.ema_model.sample(
                    ...
                    torch.save(
                        {
                            ...
                            "weights": flow_ema.online_model.state_dict(),
                            "ema_weights": flow_ema.ema_model.state_dict(),
