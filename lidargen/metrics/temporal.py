...
import torch
sys.path.append('../')
from lidargen.metrics.modules.chamfer3D.dist_chamfer_3D import chamfer_3DDist
chamfer_3D_module = chamfer_3DDist()
...
def convert_ndarray2tensor(data):
    if isinstance(data, np.ndarray):
        data = torch.from_numpy(data).to('cuda')
    return data

def compute_pairwise_cd_batch(reference, samples):
    reference = convert_ndarray2tensor(reference).unsqueeze(0).float()
    samples = convert_ndarray2tensor(samples).unsqueeze(0).float()
    ndim = reference.ndim
    assert ndim in [2, 3]
    dist_r, dist_s, _, _ = chamfer_3D_module(reference, samples)
...
def calculate_single_sequence_TCD(seq_path, data_infos):
    seq_results = defaultdict(list)
    frame_files = natsorted(os.listdir(seq_path))
    frame_files = [os.path.join(seq_path, f) for f in frame_files]
    frame_num = len(frame_files)
    ...
    for split in [1, 2, 3, 4]:
        for frame_id in range(frame_num - split):
            pts_source = load_points(frame_files[frame_id], data_infos, to_global=True)
            pts_target = load_points(frame_files[frame_id + split], data_infos, to_global=True)
            dist1, dist2 = compute_pairwise_cd_batch(pts_source, pts_target)
            seq_results[split].append(((dist1.mean() + dist2.mean())/2).item())
...
def calculate_TCD(method_name, num_threads=None):
    data_infos = pickle.load(open(f'../data/infos/nuscenes_infos_val.pkl', 'rb'))
    data_infos_dict = {info['token']: info for info in data_infos['infos']}
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
        future_to_seq = {
            executor.submit(calculate_single_sequence_TCD, seq_path, data_infos_dict): seq_path
            for seq_path in seq_paths
        }
...
if __name__ == '__main__':
    ...
    calculate_TCD(args.method_name, args.num_threads)