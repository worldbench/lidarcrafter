...
class MeanFlow(nn.Module):
    ...
    def loss(self, x, c=None):
        ...
        model_partial = partial(self.model, condition=None)
        jvp_args = (
    ...
    def forward(self, batch):
        ...
        loss, mse_val = self.loss(x, c)
        return loss, mse_val

    @torch.no_grad()
    def sample(self, device='cuda'):

        z = torch.randn(1, self.channels,
                        self.image_size[0], self.image_size[1], device=device)

        t = torch.ones((1,), device=device)
        r = torch.zeros((1,), device=device)

        z = z - self.model(z, t, r)
